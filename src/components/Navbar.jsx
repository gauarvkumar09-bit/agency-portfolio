...
const Navbar = ({theme, setTheme}) => {
  ...
  return (
    ...
    top-0 z-20  backdrop-blur-xl font-medium bg-white/100 dark:bg-gray-900/70 '>
      
       <img src={theme === 'dark' ? assets.logo_dark :assets.logo } className='w-32 sm:w-40' alt=''/>

         <div className={`text-grey-700 dark:text-black sm:text-sm ${!sidebarOpen ? "max-sm:w-0" : "max-sm:w-60 max-sm:pl-10"}
         max-sm:fixed top-0 buttom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col 
         max-sm:bg-blue-500 max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>

         <img src={assets.close_icon} alt='' className='w-6 absolute right-4 top-4 sm:hidden' onClick={()=>setSidebarOpen(false)}/>
         ...
         bg-primary text-grey px-6 py-2 rounded-full cursor-pointer hover:scale-105 transition-all' >
